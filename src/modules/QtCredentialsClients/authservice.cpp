...
#include <QUrl>
// #include <keychain.h> // Modern cross-platform keyring API
...
void AuthService::login(const QString &username, const QString &password)
{
    ...
    // 2. Format validation: Support both conventional usernames (3+ chars) and valid RFC emails
    bool isEmail = username.contains('@') && username.contains('.');
    QRegularExpression usernameRegex("^[a-zA-Z0-9_.-]{3,64}$");
    bool isValidUsername = usernameRegex.match(username).hasMatch();

    if (!isEmail && !isValidUsername) {
        emit rawLog("❌ Validation failed: Invalid username or email pattern: " + username, "error");
        emit errorOccurred("Credentials verification failed: Please provide a valid conventional username (3+ chars) or email address.");
        return;
    }
 ...
 * @details Dispatches an asynchronous QKeychain::WritePasswordJob targeting service "QtCredentialsClient"
...
void AuthService::saveTokenSecurely(const QString &token)
{
    ...
    QKeychain::WritePasswordJob *job = new QKeychain::WritePasswordJob("QtCredentialsClient", this);
...
bool AuthService::checkSavedSession()
{
    ...
    QKeychain::ReadPasswordJob *job = new QKeychain::ReadPasswordJob("QtCredentialsClient", this);
    job->setKey("session_auth_token");
...
{
    QKeychain::DeletePasswordJob *job = new QKeychain::DeletePasswordJob("QtCredentialsClient", this);
    job->setKey("session_auth_token");
