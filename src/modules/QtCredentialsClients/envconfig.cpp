...
/**
 * @brief Constructor: Initializes default connection values for internal security gateways.
 */
...
/**
 * @brief Inspects global process variables and reads localized .env.security files.
 * @details Implements a dual-layer credentials loader:
 *          1. Queries active process variables via QProcessEnvironment to support cloud container runs.
 *          2. Locates and reads an OS-isolated config directory mapped by QStandardPaths for local desktop overrides.
 */
void EnvConfig::loadFromSystem()
{
    ...
    // ---------------------------------------------------------
    // Layer 1: Read Process-Level System Environment Variables
    // ---------------------------------------------------------
    
    // Map central enterprise authorization gateway endpoints
    ...
    // Map network timeout parameters
    ...
    // Map execution environment stages (development, staging, or production)
    ...
    // ---------------------------------------------------------
    // Layer 2: Secure Platform-Independent AppConfig Directory Check
    // ---------------------------------------------------------
    
    // QStandardPaths::AppConfigLocation resolves securely to:
    ...
    QDir dir(configDir);
    if (!dir.exists()) {
        dir.mkpath("."); // Force-create configuration path recursively
    }
    ...
    // Attempt to open and parse standard key-value layout within .env.security
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!in.atEnd()) {
            ...
            if (parts.size() >= 2) {
                ...
                QString val = parts.mid(1).join("=").trimmed(); // Handle values containing '='
                m_configMap[key] = val;
                
                // Defensive checking: Do NOT spill sensitive tokens or secrets to log streams!
                if (key.contains("SECRET") || key.contains("TOKEN") || key.contains("PASSWORD") || key.contains("KEY")) {
...
/**
 * @brief Binds an internal configuration key with a system variable and a default value.
 */
...
/**
 * @brief Retrieves a previously mapped configuration parameter value by key.
 */
...
/**
 * @brief Resolves custom standard operating system directories dynamically.
 * @param folderName Identifies targeted folder profile ("config", "cache", or "data").
 */
